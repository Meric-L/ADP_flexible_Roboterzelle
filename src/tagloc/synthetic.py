"""Synthetische Szenen rendern: Bilder mit **bekannter** Wahrheit, ohne Kamera.

Grundlage von `tests/test_tag_pipeline.py` und der einzige Weg, die ganze
Kette (Detektor -> Pose -> Platzierung -> Modulpose) gegen Referenzwerte statt
gegen sich selbst zu pruefen: Tag-Posen werden hier vorgegeben, mit der echten
Intrinsik projiziert und dann von der Bibliothek zurueckgewonnen.

Lag bis zum Refactoring in `tools/make_synthetic_scene.py`; das ist jetzt nur
noch die Kommandozeile davor (und exportiert die Namen von hier weiter). Im
Paket, damit Tests es regulaer importieren koennen statt ueber `tools/` mit
`sys.path`-Vorspann.

Rendered with `cv2.aruco.generateImageMarker` (fallback `drawMarker`, see
`detector.generate_marker`) and `cv2.warpPerspective`; corner points come
from `cv2.projectPoints` using exactly the intrinsics also written to the
calibration file. That's what makes the image match the calibration, and only
that makes the reference value a reference value.

`cv2` is imported only inside functions -- importing this module works
without OpenCV.
"""
...
from .boards import BoardSpec, chessboard_object_points
...
def marker_patch(family: str, tag_id: int, marker_px: int, quiet_px: int):
    ...
    **outer edge** of the black frame (hence the half pixels) -- that's the
    ...
    """
    from .detector import aruco_dictionary, generate_marker
