...
| D2 | **Kaputtes `.venv/`** — inzwischen anders kaputt: `asyncua` ist da, aber `numpy/__init__.py` und `cv2/__init__.py` **fehlen**, beide Pakete sind leere Namespace-Pakete | Zelle: `.venv/` | numpy und OpenCV nicht importierbar. Neu anlegen, siehe [`apriltag-e2e-test.md`](apriltag-e2e-test.md) Abschnitt 0.1. Der Code umgeht das nicht: `tagloc.detector`/`boards` haben Laufzeitweichen für alte und neue `aruco`-API, aber ohne installiertes OpenCV läuft keine Erkennung |
| ~~D3~~ | ~~`requirements.txt` bildet die Realität nicht ab~~ | — | **Teilweise erledigt**: die Datei erklärt jetzt, warum `picamera2` per apt kommt (und dass die venv `--system-site-packages` braucht) und wozu `pupil-apriltags` noch dient — es ist seit `tagloc.detector` ein optionales zweites Backend, kein Zwang mehr. Ein Upper Bound auf OpenCV fehlt weiterhin bewusst, weil die Laufzeitweichen 4.x und 5.x abdecken |
