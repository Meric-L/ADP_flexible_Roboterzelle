...
| ~~C3~~ | ~~`vision_system_id` auf beiden Pis identisch~~ | — | — | **Erledigt**: `vision_identity()` in `src/vision_server/cell_server.py` (Env → Hostname-Abbildung → Fallback). Die Hostnamen in `PI_IDENTITIES` sind noch geraten und auf den Pis zu prüfen |
...
| D5 | **systemd-Unit und venv sind nirgends versioniert** | — | `opcua-server.service`, der Repo-Pfad auf dem Pi, die Python-Version und die venv-Erzeugung stehen in keinem Repo. Neuaufsetzen eines Pi ist derzeit undokumentiert. **Seit 22.09.2026 akut:** Die Unit startet `src/OPCUA/server.py` — diesen Pfad gibt es nicht mehr. Auf beiden Pis muss `ExecStart` auf `python3 -m vision_server.cell_server` mit `WorkingDirectory=<repo>/src` umgestellt werden, sonst startet der Dienst nach dem nächsten Pull nicht mehr |
