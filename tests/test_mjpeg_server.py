...
CONFIG = CameraStreamConfig(stream_fps=20.0, http_fps=100.0, overlay_mode="off")
...
class PublisherFanOutTest(unittest.IsolatedAsyncioTestCase):
    async def test_node_stays_at_stream_fps_while_viewers_run_faster(self):
        camera = FakeCamera()
        node = FakeNode()
        config = CameraStreamConfig(stream_fps=10.0, http_fps=100.0, overlay_mode="off")
        ...
        # ~0.5 s bei 10 fps: grob 5 Knotenwerte, auf keinen Fall einer pro Frame.
        self.assertLessEqual(len(node.written), 10)
